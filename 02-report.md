...
**工程 Insight**：
> 搜索 timeout 从 15s 提到 60s 只解决了配置层问题。真正的瓶颈常在 query expansion、LLM rerank、跨 collection 聚合等更深路径。搜索系统需要区分：索引是否更新、embedding 是否覆盖、query path 是否稳定、ranking 是否符合预期。对于主动召回这类前置链路，宁可在明确时限内返回空结果，也不要让一次慢检索拖住整轮对话。
...
- 对直连检索路径要设置短超时护栏，并在 debug metadata 里标记 timeout / empty-result 原因，让空结果可解释、可追踪
...
- **直连检索没有 timeout guard 会产生长尾卡顿**：direct path 可以绕开一部分调度开销，但也必须配短超时护栏；超时返回 empty 通常比把用户请求挂住更可控
...
- **cron payload 用绝对路径或显式 workspace 变量**：周期性自反思任务里写相对路径会被不同 cwd 解析到不同 workspace，造成同一 agent 的内容分裂到多处。统一用明确的 workspace root / 环境变量 / 配置项，不要依赖启动目录
...
诚实地说，把九层塔落到生产时，光配置是不够的。前文 L4（检索 CLI 输出容错与结果字段 fallback）、L5（主动召回短超时与空结果可解释）和 L6（摘要前敏感信息清理 / 短源跳过 LLM）提到的几处问题，本质上都是第三方组件与本地运行环境之间的适配问题。

这一节不再重复"改什么"，重点谈"怎样让这种修改活下去"——临时 hotfix 会随着 npm 升级或上游 git pull 失效；只有做成可观测、可重启自愈的工程资产，才能真正成为架构的一部分。最佳实践是：

| 要素 | 做法 |
|---|---|
| sentinel / marker | 在 patched 区域留 `// PATCH:<name>` 或可 grep 的 marker，apply 脚本据此判断幂等 |
...
更通用的做法，是把这些适配沉淀成 post-upgrade reinstaller，并接入服务启动前检查：它不需要依赖某一次手工 hotfix 的记忆，而是自动验证关键行为（CLI 输出格式是否稳定、结果字段是否兼容、主动召回是否有短超时等）是否仍在。包升级、npm reinstall 或 dist 重建后，下一次服务启动会先检查并尝试重打；如果上游结构变化导致 anchor 找不到，它应该 fail-closed 留日志，而不是冒险改坏产物。
...
| 3 | 检索 CLI stdout 非 JSON 噪声 | L4 | 检索类 CLI 在异常时可能往 stdout 写 warning，下游 parser 必须 fail-soft |
| 4 | 多 collection 路径单点失败传染 | L4/L5 | 一个 collection 的子查询抛错不应让整条主动召回退化到慢 fallback |
| 5 | active-memory 注入低质量摘要 / 直连检索长尾 | L5 | 加 minRelevanceScore + untrusted context 标记；direct path 必须有短 timeout guard，超时返回 empty 并记录原因 |
